  ...
  return (
    ...
    <main>
        <header className='fixed top-6 left-6 flex space-x-3 items-center'>
            <button  onClick={()=>{navigate(-1)}}className=" border border-neutral-50 shadow-sm flex justify-center items-center p-2 rounded-md text-purple-600">
                <ChevronLeft/>
                
            </button>
            <div className="flex flex-col">
            <h1 className='text-lg text-purple-700 font-[700] tracking-wide'>Instructors of Acquel</h1>
        ...
        <section className='flex flex-col space-y-10 px-5 mt-24'>
           ...
           !isLoading && instructors ? <> {
            instructors?.map((prof,i)=>(
                <InstructorItem key={i} noOfCourses={prof.courses.length!}profileUrl={prof.profileUrl} name={getUserName(prof.email)!} email={prof.email} verified={prof.canCreateCourse}/>
            ))
           }</> : <>Hello</>
           }
        </section>
