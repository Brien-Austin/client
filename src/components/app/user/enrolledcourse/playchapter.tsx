...
import { USER_FETCH_CHAPTER } from "@/utils/constants";
...
const PlayChapter = () => {
  ...
  const { data: chapter } = useQuery<Chapter>({
    queryKey: ['fetch-chapter', id],  // Added id to queryKey
    queryFn: async () => {
  ...
  return (
    ...
    <nav className="fixed bottom-0 left-0 h-16 w-full  ">
      <div className="px-5 flex justify-between items-center">
        ...
        <button className="bg-gradient-to-r from-purple-600 via-purple-500 text-white px-3 py-2 to-purple-700 rounded-full ring-1 ring-purple-400">
          Mark as complete
        </button>
